...
const LAZY_VERIFICATION_THRESHOLD = 2 * 60 * 60 * 1000; // Trust local for 2 hours
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      ...
      try {
        ...
        // LAZY VERIFICATION: Only verify with server if token was created > threshold ago
        // or if we're feeling extra safe. For fast login, we trust local storage if fresh.
        const decoded = decodeToken(storedUser.token);
        const issuedAt = decoded?.iat ? decoded.iat * 1000 : Date.now();
        const isFresh = (Date.now() - issuedAt) < LAZY_VERIFICATION_THRESHOLD;

        if (isMounted) {
          setIsLoading(false); // Let the app render immediately
        }

        if (isFresh) {
          console.log('Login: Using fresh cached session (Lazy Verification)');
          return; // Skip server call!
        }

        // Verify token in background if not fresh
        const verifiedUser = await verifyTokenApi();
  ...
  useEffect(() => {
    ...
    }, 300000); // 5 minute heartbeat is sufficient (reduced from 1m)
