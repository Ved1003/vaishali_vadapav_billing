...
export default function AdminDashboard() {
  ...
  const { data: billsData } = useQuery({ queryKey: ['recent-bills'], queryFn: () => getBillsApi({ limit: 6 }) });
  ...
  const handleBillerClick = async (biller: BillerRevenue) => {
    ...
    try { 
      const data = await getBillsApi({ billerId: biller.billerId, limit: 1000 });
      setBillerBills(data.bills); 
    }
                ...
                {recentBills.length === 0 ? (
                  ...
                  <div className="flex-1 overflow-y-auto">
                    <AnimatePresence mode="popLayout">
                      {recentBills.map((bill: Bill, i: number) => (
