...
import { User, AuthUser, Item, Bill, DashboardStats, BillerRevenue, DailyRevenue, FridgeItem, PaginatedBills } from '@/types';
...
export const getBillsApi = async (filters: any = {}): Promise<PaginatedBills> => {
  const queryParams = new URLSearchParams();
  ...
  return apiCall<PaginatedBills>(`/bills?${queryParams.toString()}`);
};
