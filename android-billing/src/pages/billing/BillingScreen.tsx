...
export default function BillingScreen() {
    ...
    const addItem = useCallback((item: Item, e?: React.MouseEvent | React.TouchEvent) => {
        ...
        const now = Date.now();
        if (e && 'currentTarget' in e && (now - lastAnimationTime.current > 150)) {
    ...
    const saveBill = async (paymentMode: 'cash' | 'upi') => {
        ...
        // OPTIMISTIC UI: Capture state, clear cart immediately and celebrate
        const itemsToSave = [...billItems];
        const totalToSave = grandTotal;
        
        setBillItems([]);
        setCartOpen(false);
        setPulse(false);

        // Celebration (Optimized for performance)
        confetti({
        ...
        try {
            ...
            toast({
                title: '✅ Bill Created',
                description: `Bill #${bill.billNumber} – ₹${totalToSave.toFixed(2)}`,
            ...
            // RECOVERY: If server fails, restore items so they aren't lost
            setBillItems(itemsToSave);
            setCartOpen(true);
            const _msg = error instanceof Error ? error.message : 'Network Error';
            toast({ 
                title: '❌ Sync Failed', 
                description: 'Bill not saved. Your cart has been restored. Try again.', 
                variant: 'destructive',
                duration: 5000
            });
        } finally {
